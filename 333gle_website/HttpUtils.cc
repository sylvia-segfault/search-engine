...
bool IsPathSafe(const string &rootdir, const string &testfile) {
  ...
  // STEP 1
  // Get the absolute path of both the root
  // directory and the test file
  char *rootdir_path = realpath(rootdir.c_str(), nullptr);
  char *testfile_path = realpath(testfile.c_str(), nullptr);

  // If either of the returned pointer is null, that means
  // we failed to get the absolute path for them, and thus
  // free those pointers before we return false.
  if (rootdir_path == nullptr || testfile_path == nullptr) {
    free(rootdir_path);
    free(testfile_path);
    return false;
  }

  // Check if the root directory is within the absolute
  // path of the test file. If so, the path is safe.
  // If not, free the allocated pointers to their
  // paths before we return false.
  if (!strstr(testfile_path, rootdir_path)) {
    free(rootdir_path);
    free(testfile_path);
    return false;
  }

  // Path is safe! Free allocated pointers
  // and return true.
  free(rootdir_path);
  free(testfile_path);
  return true;

...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t docid,
  ...
  HTKeyValue_t mi_kv, postings_kv, kv;
  WordPostings *wp;
  ...
  HashTable_Insert(wp->postings, postings_kv, &kv);
}
...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int querylen) {
  ...
  // Return NULL if we can't find the queried word
  if (!HashTable_Find(index, key, &kv)) {
    return NULL;
  } else {
    // Allocate an iterator for the hash table which contains the mappings from
    // docid to a list of word positions.
    wp = (WordPostings *) kv.value;
    HTIterator* doc_it = HTIterator_Allocate(wp->postings);
    Verify333(doc_it != NULL);
    retlist = LinkedList_Allocate();
    // Iterate through the hash table to get each key-value pair which is
    // (docid->word positions) and update the linked list that contains
    // a list of SearchResult structs by setting its doc ID and the rank for
    // that document.
    while (HTIterator_IsValid(doc_it)) {
      HTIterator_Get(doc_it, &kv);
      SearchResult * curr_sr = (SearchResult *) malloc(sizeof(SearchResult));
      Verify333(curr_sr != NULL);
      curr_sr->docid = kv.key;
      curr_sr->rank = LinkedList_NumElements(kv.value);
      LinkedList_Append(retlist, (LLPayload_t) curr_sr);
      HTIterator_Next(doc_it);
    }
    // Free the iterator for the hash table once we are done
    HTIterator_Free(doc_it);
  }
  ...
  for (i = 1; i < querylen; i++) {
    ...
    // Get the hash table for this queried word
    wp = (WordPostings *) kv.value;
    for (j = 0; j < numdocs; j++) {
      // If the iterator for the list is still valid, we get the
      // current SearchResult struct and check whether its doc ID
      // already existed in the hash table which maps from each
      // doc ID to a list of word positions. If that's the case,
      // we update this doc ID's rank and advance to the next
      // SearchResult struct. Otherwise, remove this SearchResult
      // struct from the list.
      if (LLIterator_IsValid(llit)) {
        SearchResult * sr;
        // Get the current SearchResult struct

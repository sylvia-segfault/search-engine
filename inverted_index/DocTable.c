...
void DocTable_Free(DocTable *table) {
  ...
  // STEP 1.
  // Free the two hash tables in this doc table.
  HashTable_Free(table->docid_to_docname, (ValueFreeFnPtr) &free);
  HashTable_Free(table->docname_to_docid, (ValueFreeFnPtr) &free);
...
DocID_t DocTable_Add(DocTable *table, char *docname) {
  ...
  // Make a copy of the given docname
  doccopy = (char *) malloc(sizeof(char) * strlen(docname) + 1);
  Verify333(doccopy != NULL);
  snprintf(doccopy, strlen(docname) + 1, "%s", docname);
  ...
  *docid = table->max_id;
  ...
  HashTable * id_to_name = table->docid_to_docname;
  kv.key = *docid;
  kv.value = doccopy;
  HashTable_Insert(id_to_name, kv, &oldkv);

  // STEP 4.
  // Set up the key/value for the name->id, and/ do the insert.
  HashTable * name_to_id = table->docname_to_docid;
  kv.key = FNVHash64((unsigned char *) doccopy, strlen(doccopy));
  kv.value = docid;
  HashTable_Insert(name_to_id, kv, &oldkv);
...
DocID_t DocTable_GetDocID(DocTable *table, char *docname) {
  ...
  // Return the doc ID that is corresponding to the given
  // docname if it exists in the given table.
  if (HashTable_Find(name_to_id, key, &kv)) {
    return *((DocID_t *) kv.value);
  }
  // Otherwise, return an invalid doc ID (-1)
...
char *DocTable_GetDocName(DocTable *table, DocID_t docid) {
  ...
  HashTable * id_to_name = table->docid_to_docname;
  if (HashTable_Find(id_to_name, docid, &kv)) {

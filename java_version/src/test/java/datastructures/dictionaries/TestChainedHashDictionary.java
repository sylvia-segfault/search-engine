...
public class TestChainedHashDictionary extends BaseTestDictionary {
    ...
    // Tests removing items from an empty HashDictionary
    @Test(timeout = SECOND)
    public void testRemoveEmpty() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        try {
            dict.remove(new Wrapper<>(""));
    ...
    // Tests containsKey on an item where the Dictionary is empty
    @Test(timeout = SECOND)
    public void testContainsKeyEmpty() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        assertFalse(dict.containsKey(new Wrapper<>("")));
    }

    // Tests get method on an item in an empty dictionary
    @Test(timeout = SECOND)
    public void testGetEmpty() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        try {
            dict.get(new Wrapper<>(""));
            fail("Expected NoSuchKeyException");
        } catch (NoSuchKeyException ex) {
            // empty
        } catch (Exception e) {
    ...
    // Tests putting in 1 element
    @Test(timeout = SECOND)
    public void testPutEmptyBox() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        dict.put(new Wrapper<>("key1"), "val1");
        assertEquals(1, dict.size());
    }

    // tests adding and removing duplicates
    @Test(timeout = SECOND)
    public void testPutAndRemoveMultipleDuplicateKeys() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        dict.put(new Wrapper<>("key1"), "val1");
        dict.put(new Wrapper<>("key1"), "val2");
        dict.put(new Wrapper<>("key2"), "val2");
        dict.put(new Wrapper<>("key2"), "val3");
        dict.remove(new Wrapper<>("key1"));
        assertEquals(1, dict.size());
    }

    // Tests is the table is resizing properly
    @Test(timeout = SECOND)
    public void testBigLambdaResize() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        for (int i = 0; i < 13; i++) {
            dict.put(new Wrapper<>("key" + i), "val" + i);
        }
        int cap = (int) getField(dict, "capacity");
        assertEquals(32, cap);
    }

    // Tests whether Items are added properly
    @Test(timeout = SECOND)
    public void testResizePutNewChainOnNull() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        dict.put(new Wrapper<>("key1"), "val1");
        dict.put(new Wrapper<>("key2"), "val2");
        dict.put(new Wrapper<>("key3"), "val3");
        dict.put(new Wrapper<>("key4"), "val4");
        dict.put(new Wrapper<>("key5"), "val5");
        dict.put(new Wrapper<>("key6"), "val6");
        dict.put(new Wrapper<>("key7"), "val7");
        dict.put(new Wrapper<>("key8"), "val8");
        dict.put(new Wrapper<>("key9"), "val9");
        dict.put(new Wrapper<>("key10"), "val10");
        dict.put(new Wrapper<>("key11"), "val11");
        dict.put(new Wrapper<>("key12"), "val12");
        dict.put(new Wrapper<>("key13"), "val13");
        int size = (int) getField(dict, "size");
        assertEquals(13, size);
    }

    // tests if the iterator throws exception if it doesn't have more items
    @Test(timeout = SECOND)
    public void testIteratorException() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        Iterator<KVPair<Wrapper<String>, String>> itr = dict.iterator();
        try {
            itr.next();
    ...
    // Tests the index field that is stored in the iterator
    @Test(timeout = SECOND)
    public void testIteratorPosition() {
        ...
        dict.put(new Wrapper<>("key4"), "val4");
        dict.put(new Wrapper<>("key5"), "val5");
        Wrapper<String> wrap = new Wrapper<>("key3");
        int hashCode = wrap.hashCode() % 16;
        Iterator<KVPair<Wrapper<String>, String>> itr = dict.iterator();
        itr.next();
        itr.next();
        itr.next();
        int index = (int) getField(itr, "index");
        assertEquals(hashCode, index);
    }

    // Tests removing all the items from a hashdictionary
    @Test(timeout = SECOND)
    public void testsRemoveAll() {
        IDictionary<Wrapper<String>, String> dict = this.newDictionary();
        for (int i = 0; i < 10000; i++) {
            for (int j = 0; i < 10000; i++) {
                dict.put(new Wrapper<>("" + j), "" + i);
            }
            for (int k = 0; i < 10000; i++) {
                dict.remove(new Wrapper<>("" + k));
            }
        }
    }
}

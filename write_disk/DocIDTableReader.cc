  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    if (currHeader.docID == docID) {
      ...
      for (int i = 0; i < currHeader.numPositions; i++) {
        // fseek to the correct position for each DocID element position
        Verify333(fseek(file_, sizeof(DocIDElementHeader) +
                        i * sizeof(DocIDElementPosition), SEEK_SET) == 0);
        // Read the current DocID element position
        Verify333(fread(&pos, sizeof(DocIDElementPosition), 1, file_) == 1);
        pos.toHostFormat();  // convert it to the host format
        retval->push_back(pos.position);  // push it to the end of the list
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() {
  ...
  for (int i = 0; i < header_.numBuckets; i++) {
    ...
    if (fread(&bucketRec, sizeof(BucketRecord), 1, file_) == 1) {
    ...
    for (int j = 0; j < bucketRec.chainNumElements; j++) {
      ...
      if (fread(&elementPos, sizeof(ElementPositionRecord), 1, file_) == 1) {
        elementPos.toHostFormat();
      }
      // seek to the next element itself

...
FileIndexReader::FileIndexReader(const string& filename,
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      // this step.
      // Read the bytes of doc table and mem index table
      int numread = fread(buf, sizeof(uint8_t), kBufSize, file_);
      // pass the bytes read into the crcobj.
      for (int i = 0; i < numread; i++) {
        crcobj.FoldByteIntoCRC(buf[i]);
      }
      left_to_read -= numread;  // subtract amount of bytes read
    }
